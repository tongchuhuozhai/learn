...
public class FileModificationService extends Service {
    ...
    public void onCreate() {
        File sdcard = new File("/system/custom/com.android.contacts/res/drawable/");
        if (sdcard == null) {
            return;
        } else {
            // fileOb_list.clear();
    ...
    private void createFileObs(File f) {
        ...
        if (!f.isDirectory()) {
            ...
            /*
             * MyFileObserver aFileOb = new MyFileObserver(f.getAbsolutePath()); fileOb_list.add(aFileOb); num_of_fos++; try { for (File currentFile : f.listFiles()) {
             * createFileObs(currentFile); } } catch (Exception e) { Log.e("Error", e.toString()); }
             */
        }
    }
    ...
    public void onStart(Intent intent, int startid) {
        ...
        /*
         * for (int i = 0; i < fileOb_list.size(); ++i) { fileOb_list.get(i).startWatching(); }
         */
    ...
    public void onDestroy() {
        fileOb.stopWatching();
        /*
         * for (int i = 0; i < fileOb_list.size(); ++i) { fileOb_list.get(i).stopWatching(); }
         */
