...
public class ContactActivity extends ListActivity {
	...
	public void onCreate(Bundle bundle) {
		...
		((TextView) findViewById(R.id.right_text_title)).setText(R.string.contact_title);
		((ImageView) findViewById(R.id.back_icon)).setOnClickListener(new OnClickListener() {
			@Override
	...
	private void setupView() {
		emptyView = LayoutInflater.from(this).inflate(R.layout.contact_wait, null);
			...
			public void onClick(View v) {
				...
				if (personsArrayList != null && contactFlag) {
					...
					if (count > 0) {
						for (int i = 0; i < count; i++) {
							if (selectedArray[i] == true) {
								if (selectedPersons.length() == 0) {
									selectedPersons.append((String) personsArrayList.get(i));
								} else {
									selectedPersons.append(',' + (String) personsArrayList.get(i));
								}
							}
						...
						if (selectedPersons.length() == 0) {
							Toast.makeText(ContactActivity.this, R.string.select_contacts, 3000).show();
						} else {
							// 向调用页返回数据
							Intent intent = getIntent();
							intent.putExtra("selected Persons", selectedPersons.toString());
							logger.i("selectedPersons is: " + selectedPersons.toString());
							setResult(RESULT_OK, intent);
							finish();
						}
					} else {
						Toast.makeText(ContactActivity.this, R.string.no_contact, 3000).show();
					}
				}
			}
		});
	...
	private void searchContacts(Uri mContacts) {
		...
		Cursor cur = resolver.query(mContacts, columns, null, null, People.NAME);
		if (cur.moveToFirst()) {
			do {
				...
				if (temp != null && name != null) {
					...
					if (checkMobilePhone(phoneNo)) {
						// 截取手机号(即去掉+86)
						Pattern pattern = Pattern.compile("1(3[4-9]|5[012789]|8[78])\\d{8}$");
						...
						if (!phoneMap.containsKey(phoneNoMachedString)) {
							if (name != null && !"".equals(name)) {
								personsArrayList.add(phoneNoMachedString.concat("[").concat(name).concat("]"));
	...
	private void setupListContent(boolean[] selectedArray, ArrayList<String> persons) {

		if (personsArrayList.size() == 0) {
			emptyView.findViewById(R.id.process_bar).setVisibility(View.GONE);
			emptyView.findViewById(R.id.no_content_tip).setVisibility(View.VISIBLE);
		} else {
			if (persons != null && selectedArray != null) {
				setListAdapter(new ContactAdapter(ContactActivity.this, persons, selectedArray, handler));
			} else {
	...
	private static boolean checkMobilePhone(String phone) {
		 ...
		 */
		Pattern pattern = Pattern.compile("^(\\+86)?1(3[4-9]|5[012789]|8[78])\\d{8}$");
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case CHECK_CHANGEED:
				...
				selectedArray[personsListView.getPositionForView((View) msg.obj)] = ((CheckBox) view.findViewById(R.id.contact_check)).isChecked();
				break;
			}
		}
	};
